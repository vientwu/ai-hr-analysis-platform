// API 配置：本地联调优先走备用后端；线上默认直连 Coze（使用浏览器本地保存的 PAT）
...
async function uploadToCoze(file, pat) {
  if (!pat) throw new Error('未设置 Coze PAT，请先在本机保存 COZE_PAT');
...
async function runWorkflowResume(fileId, jd, pat) {
  ...
  const payload = {
    workflow_id: wf,
    parameters: { files: [JSON.stringify({ file_id: fileId })], JD: jd || '' },
  };
...
async function runWorkflowInterview(fileId, name, recordingUrl, pat) {
  ...
  const payload = {
    workflow_id: wf,
    parameters: { recording_file: JSON.stringify({ file_id: fileId }), name: name || '', recording_url: recordingUrl || '' },
  };
  const resp = await fetch('https://api.coze.cn/v1/workflow/run', {
    method: 'POST',
    headers: { Authorization: `Bearer ${pat}`, 'Content-Type': 'application/json', Accept: 'application/json' },
    ...
    try {
        ...
        if (!API_BASE) {
            ...
            const response = await fetch(API_CONFIG.endpoints.resume, {
                ...
                body: JSON.stringify({ fileName: resumeFile.name, fileBase64, jd: jobDescription })
            });
    ...
    try {
        ...
        if (!API_BASE) {
            ...
            const response = await fetch(API_CONFIG.endpoints.interview, {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ fileName: transcriptFile.name, fileBase64, name: intervieweeInfo, recordingUrl: recordingLink || '' })
            });
            if (!response.ok) { throw new Error(`API请求失败: ${response.status} ${response.statusText}`); }
            return await response.json();
        }
...
function formatAPIResponse(response) {
    try {
        if (response.data) {
            return { success: true, data: response.data, message: response.message || '分析完成' };
        } else if (response.result) {
            return { success: true, data: response.result, message: '分析完成' };
        } else {
            return { success: true, data: JSON.stringify(response, null, 2), message: '分析完成' };
        }
    } catch (error) {
