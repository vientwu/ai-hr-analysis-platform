...
async function handleForgotPassword() {
    ...
    try {
        if (window.Auth) {
            ...
            if (result.error) {
                showLoginError(result.error.message || '发送验证码失败');
            } else {
...
async function handleVerifyOtp() {
    ...
    try {
        if (window.Auth && typeof window.Auth.verifyOtp === 'function') {
            const result = await window.Auth.verifyOtp(email, otp, 'recovery');
            if (result.error) {
                showLoginError(result.error.message || '验证码错误或已过期');
            } else {
                closeLoginModal();
            ...
            showLoginError('认证服务不支持验证码');
        }
    } catch (error) {
        console.error('Verify OTP error:', error);
        showLoginError('验证失败，请稍后重试');
    }
}
...
function switchLoginMode(mode) {
    ...
    const verifyActions = document.getElementById('verify-otp-actions');
    const rememberGroup = document.getElementById('login-remember-group');
    const subtitle = document.querySelector('#login-modal .modal-subtitle');
    const helpLinks = document.querySelector('#login-modal .modal-help');
    ...
    if (mode === 'forgot') {
        ...
        if (helpLinks) helpLinks.style.display = 'none';
        if (subtitle) subtitle.textContent = '输入邮箱以接收验证码';
    } else if (mode === 'otp') {
        if (pwdGroup) pwdGroup.style.display = 'none';
        if (rememberGroup) rememberGroup.style.display = 'none';
        if (loginActions) loginActions.style.display = 'none';
        ...
        if (helpLinks) helpLinks.style.display = 'none';
        if (subtitle) subtitle.textContent = '输入您邮箱收到的6位验证码';
    } else {
...
if (typeof handleVerifyOtp === 'function') window.handleVerifyOtp = handleVerifyOtp;
